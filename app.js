...
if (snapshotList) {
  ...
  const renderBars = () => {
    ...
    filters.forEach((f) => {
      ...
      g.innerHTML = `<span>${f.name}</span><div class="bar" style="--v:${f.value}"></div><small>${f.value}</small>`;
...
if (insightBars) {
  ...
  const renderBars = () => {
    ...
    days.forEach((d, i) => {
      ...
      wrap.innerHTML = `<span>${d}</span><i style="--v:${data[i] || 1}"></i><small>${data[i] || 0}</small>`;
