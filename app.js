...
const barHeightPx = (value, minPx = 14, maxPx = 180) => {
  const safe = Math.max(1, Math.min(100, Number(value) || 1));
  return Math.round(minPx + ((safe - 1) / 99) * (maxPx - minPx));
};
...
if (income) {
  ...
  const update = () => {
    ...
    incomeVal.textContent = i; needsVal.textContent = n; wantsVal.textContent = w; saveVal.textContent = s; planned.textContent = `AED ${total}`; leftover.textContent = `AED ${left}`;
...
if (snapshotList) {
  ...
  const renderBars = () => {
    ...
    filters.forEach((f) => {
      ...
      g.innerHTML = `<span>${f.name}</span><div class="bar"></div><small>AED ${f.value}</small>`;
      g.querySelector(".bar").style.height = `${barHeightPx(f.value)}px`;
...
if (goalList) {
  ...
  const drawGoals = () => {
    ...
    goals.forEach((g, idx) => {
      ...
      card.innerHTML = `<input value="${g.name}" maxlength="20"/><div class="ring" style="--p:${pct}"><span>AED ${g.current} / AED ${g.target}</span></div><div class="inline-form"><input type="number" min="1" value="${g.target}" /><input type="number" min="0" value="${g.current}" /><button class="btn btn-primary" type="button">+AED 5</button><button type="button">Delete</button></div>`;
...
if (insightBars) {
  ...
  const renderBars = () => {
    ...
    days.forEach((d, i) => {
      ...
      const val = data[i] || 0;
      wrap.innerHTML = `<span>${d}</span><i></i><small>AED ${val}</small>`;
      wrap.querySelector("i").style.height = `${barHeightPx(val)}px`;
